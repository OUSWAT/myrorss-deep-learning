...
fields_to_accumulate = fields[1:-2]
#
...
def extract(day, OUTPATH, check=True):
    ...
    if check:  # only unzip the composite reflectivity and MESH
    ...
    for field in iterfields:
        if field == 'MergedLLShear' or field == 'MergedMLShear':
            # os.system('tar -xvf {}/{}/Azshear/{}.tar -C {}/{}/{} --wildcards
            # "{}"'.format(DATA_HOME,year,day,OUT_HOME,year,day,field)) #
            # file-naming convention is different for early years (1999) vs
            # later
            cmd = 'tar -xvf {}/{}/azimuthal_shear_only/{}.tar -C {} --wildcards "{}"'.format(
                DATA_HOME, year, day, OUT_HOME, field)
            p = sp.Popen(cmd, shell=True)
            p.wait()
        else:
            cmd = 'tar -xvf {}/{}/{}.tar -C {} --wildcards "{}"'.format(
                DATA_HOME, year, day, OUT_HOME, field)
            p = sp.Popen(cmd, shell=True)
            p.wait()
        # pointing to the field in the uncropped directory
        field_path = '{}/{}'.format(OUT_HOME, field)
        # list the directories within the field
        subdirs = os.listdir(field_path)
        for subdir in subdirs:                                     # usually a dummy directory like '00.00' or '00.25'
            files = next(walk('{}/{}'.format(field_path, subdir)),
                         (None, None, []))[2]  # only grab files
            for f in files:
                # convert from .gz to .netcdf
                filename = '{}/{}/{}'.format(field_path, subdir, f)
                if filename[:-
                            2] != 'gz':  # only convert .gz's (else an error is thrown)
                    pass
                gz = gzip.open(filename)
                data = netCDF4.Dataset(
                    'dummy', mode='r', memory=gz.read())  # open using netCDF4
                # use xarray backend to convert correctly (idk)
                dataset = xr.open_dataset(xr.backends.NetCDF4DataStore(data))
                dataset.to_netcdf(
                    '{}/{}/{}.netcdf'.format(lscratch, field, f[:15]))  # write
    return None
...
def untar(day, field, OUT_HOME):
    ...
    if field == 'MergedLLShear' or field == 'MergedMLShear':
        cmd = 'tar -xvf {}/{}/azimuthal_shear_only/{}.tar -C {} --wildcards "{}"'.format(
            DATA_HOME, day[:4], day, OUT_HOME, field)
        p = sp.Popen(cmd, shell=True)
        p.wait()
    else:
        cmd = 'tar -xvf {}/{}/{}.tar -C {} --wildcards "{}"'.format(
...
def get_valid_files(files, valid_times):
    ...
    for key in valid_files.keys():
        if not valid_files[key]:
            continue
        else:
            new_files.append(files[key])
    return new_files

# script for use by full_extract, after extract, localmax, and check_MESH
# have been run to create storms.csv
...
def extract_after(day, potential_times, OUTPATH=lscratch, target=False):
    ...
    for field in fields:
        ...
        if target:
            field_path = '{}/target_MESH_Max_30min/MESH_Max_30min'.format(
                OUT_HOME)
        else:
            field_path = '{}/{}'.format(OUT_HOME, field)
        files = glob.glob('{}/**/*.netcdf.gz'.format(field_path))
        # return the valid files from the list of actual files and the valid
        # times
        valid_files = get_valid_files(files, valid_times)
        # extract these files
        for filename in valid_files:
            ...
            print('assigning')
            dataset.to_netcdf(
                '{}/{}/{}.netcdf'.format(lscratch, field, f[:15]))
            dataset.to_netcdf(
                '{}/test/{}.netcdf'.format(UNCROPPED_HOME, f[:15]))
...
def full_extract(day, valid_times, OUTPATH):
    ...
    '''
    # first extract inputs from t0 to t1, where t1 is the time where
    # decide(MESH) == True
    print(day)
    print(valid_times)
...
def extract_nse(day, OUTPATH):
    ...
    for field in NSE_fields:
        ...
        p = sp.Popen(cmd, shell=True)
        p.wait()
        # i.e. $LSCRATCH/NSE/SRHelicity0-2km/nseanalysis
        field_path = '{}/{}/nseanalysis/'.format(nse_path, field)
        files = next(walk(field_path), (None, None, []))[
            2]  # list files in the directory
        for f in files:
            try:
                filename = '{}/{}'.format(field_path, f)
                # e.g. nseanalysis/20110401-100000.netcdf.gz (unzip using
                # gzip.open)
                ...
                dataset = xr.open_dataset(xr.backends.NetCDF4DataStore(data))
                dataset.to_netcdf(
                    path='{}/NSE/{}/{}.netcdf'.format(OUT_HOME, field, f[:15]))
...
def localmax(day, OUTPATH):
    ...
    cmd2 = 'w2localmax -i {}/code_index.xml -I MergedReflectivityQCComposite -o {}/{} -s -d "40 60 5"'.format(
        out_path, out_path, day)
    cmd3 = 'makeIndex.pl {}/{} code_index.xml'.format(out_path, day)
    cmd4 = 'w2table2csv -i {}/{}/code_index.xml -T MergedReflectivityQCCompositeMaxFeatureTable -o {}/{}/csv -h'.format(
        out_path, day, myrorss_path, day)
...
def get_storm_info(day, OUTPATH):
    ...
    files = sorted(
        glob.glob(
            '{}/MergedReflectivityQCCompositeMaxFeature**.csv'.format(LOCALMAX_PATH),
            recursive=True))
# files = sorted(next(walk('{}'.format(LOCALMAX_PATH)), (None, None,
# []))[2]) # Grab files within localmax directory (csv)
    length = len(files) - 1
    for idx, f in enumerate(files):
        timedate = f[-19:-4]
        ...
        if (int(minutes) >= 28 and int(minutes) <= 32) or (int(minutes) >= 58 and int(
            ...
            # length:
            df = pd.read_csv(
                '{}/MergedReflectivityQCCompositeMaxFeatureTable_{}.csv'.format(LOCALMAX_PATH, timedate))
            if df.empty:
...
def check_df(storms_df):
    ...
    for idx, row in storms_df.iterrows():
        ...
        if is_Storm and t0 not in valid_times:
...
def check_MESH(storms_df, day, OUTPATH):
    OUT_HOME = OUTPATH
    # Check whether the input MESH swath is above a threshold to ensure that
    # hailstorms are chosen, not fledgeling storms
    year = day[:4]  # whatever
    if not os.path.isdir('{}/MESH_Max_30min'.format(OUT_HOME, year, day)):
        cmd = 'w2accumulator -i {}/code_index.xml -g MESH -o {} -C 1 -t 30 --verbose="severe"'.format(
            OUT_HOME, OUT_HOME)
        p = sp.Popen(cmd, shell=True)
        p.wait()
    ...
    for idx, row in storms_df.iterrows():
        ...
        cmd_list = []
        cmd_list.append(
            "makeIndex.pl {} code_index.xml {} {}".format(
                OUT_HOME, time1, time2))
        cmd_list.append(
            'w2cropconv -i {}/code_index.xml -I MESH_Max_30min -o {}/test_MESH -t "{} {}" -b "{} {}" -s "0.005 0.005" -R -n'.format(
                OUT_HOME,
                OUT_HOME,
                latNW,
                lonNW,
        ...
        if files == []:
            # if empty, then skip this time in the future
            skip_times.append(time1)
       ...
       # Decide whether it meets the conditions
        ...
        # storms_df = storms_df.at[idx, 'is_Storm'] = keep # set is_Storm to
        # true or false
    print(keep_list)
    storms_df['is_Storm'] = keep_list
    storms_df.to_csv(
        '{}/{}/{}/csv/storms.csv'.format(TRAINING_HOME, year, day))
    return storms_df
...
def accumulate(day, fields, OUTPATH):
    ...
    for field in fields:
        cmd = 'w2accumulator -i {}/code_index.xml -g {} -o {} -C 1 -t 30 --verbose="severe"'.format(
            OUT_HOME, field, OUT_HOME)
        p = sp.Popen(cmd, shell=True)
        stdout, stderr = p.communicate()
        print(cmd)
        findMin = False  # I don't care about the min shear, for now
        if field[8:] == 'Shear' and findMin:  # so skip this loop
            cmd = 'w2accumulator -i {}/code_index.xml -g {} -o {} -C 3 -t 30 --verbose="severe"'.format(
                OUT_HOME, field, OUT_HOME)
            p = sp.Popen(cmd, shell=True)
            stdout, stderr = p.communicate()
    return
    ...
    for idx, row in case_df.iterrows():
        ...
        time2 = (
            ...
            datetime.timedelta(
                ...
                seconds=30)).strftime('%Y%m%d-%H%M%S')
        # crop input
        cmd = "makeIndex.pl {} code_index.xml {} {}".format(
            OUT_HOME, time1, time2)
        p = sp.Popen(cmd, shell=True)
        p.wait()
        for field in products:
            if os.path.isdir(
                    '{}/{}/storm{:04d}/{}'.format(myrorss_path, date, idx, field)):
                continue
            cmd = 'w2cropconv -i {}/code_index.xml -I {} -o /{}/{}/storm{:04d} -t "{} {}" -b "{} {}" -s "0.005 0.005" -R -n '.format(
                OUT_HOME, field, myrorss_path, date, idx, latNW, lonNW, latSE, lonSE)
            p = sp.Popen(cmd, shell=True)
            p.wait()
        ...
        cmd1 = "makeIndex.pl {} code_index.xml {} {}".format(
            OUT_HOME, time1, time2)
        cmd2 = 'w2cropconv -i {}/code_index.xml -I MESH_Max_30min -o {}/{}/storm{:04d}/target_MESH_Max_30min -t "{} {}" -b "{} {}" -s "0.005 0.005" -R -n'.format(
            OUT_HOME, myrorss_path, date, idx, latNW, lonNW, latSE, lonSE)
        cmd3 = "makeIndex.pl {}/NSE code_index.xml {}0000 {}0000".format(
            OUT_HOME, time1[:11], time2[:11])
        ...
        for f in NSE_fields:
            if os.path.isdir(
                    '{}/{}/storm{:04d}/NSE/{}'.format(myrorss_path, date, idx, f)):
                continue
            cmd = 'w2cropconv -i {}/NSE/code_index.xml -I {} -o {}/{}/storm{:04d}/NSE -t "{} {}" -b "{} {}" -s "0.005 0.005" -R -n --verbose="severe"'.format(
                OUT_HOME, f, myrorss_path, date, idx, latNW, lonNW, latSE, lonSE)
            p = sp.Popen(cmd, shell=True)
            p.wait()
    return 'happy meal'
...
#    samples = pd.read_csv('/condo/swatwork/mcmontalbano/MYRORSS/data/2011/20110326/csv/storms.csv')
    ...
    # accumulate the correct storms
    accumulate(day, fields_to_accumulate, OUT_PATH)
    t6 = time.time()
